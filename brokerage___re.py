...
prompt = f"""
You are a financial news analyst tasked with identifying recent high-impact corporate news in India, focusing on stock which is highly recommendade by top brokerage fund. Use only information available through Google Search from the last 24 hours and prioritize relevance and credibility.

Search for:
Jefferies OR Nomura OR ICICI Direct OR CLSA OR Goldman Sachs OR Morgan Stanley OR JPMorgan OR HSBC Securities OR Citi Research OR UBS OR Credit Suisse OR Macquarie OR Axis Securities OR HDFC Securities OR Motilal Oswal OR Kotak Securities OR SBI Securities OR IIFL Securities OR Sharekhan OR Edelweiss OR Zerodha OR Upstox OR Angel One OR Groww OR Religare Broking OR Ventura Securities OR Anand Rathi OR 5Paisa OR Geojit OR Nuvama Wealth OR BNP Paribas Securities OR Deutsche Bank OR Bernstein Research OR Barclays Capital
(Jefferies OR Nomura OR ICICI Direct OR CLSA OR Goldman Sachs) AND (buy OR sell OR downgrade OR upgrade OR target OR initiate coverage OR investment call OR rating)
(Jefferies OR Nomura OR ICICI Direct OR CLSA OR Goldman Sachs) AND (order cancelled OR investment withdrawn OR deal cancelled OR tender cancelled)

i have gave you all query in one but you will have to fragment this to search easly
        ...
        response = client.models.generate_content(
            ...
            config=GenerateContentConfig(
                ...
                thinking_config=genai.types.ThinkingConfig(
                    ...
                    thinking_budget=10000    # Set thinking budget
                ),
